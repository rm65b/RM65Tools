...
---collect_data.py 标定数据采集程序(两种模式通用)

---compute.py 标定计算程序(按config.yaml的calib_mode自动选择眼在手上/眼在手外)

---config.yaml 标定板参数 + calib_mode(eye_in_hand/eye_to_hand)

---libs/poses.py 位姿->齐次矩阵转换(统一,带invert参数)
...
### compute.py 关键代码解释（眼在手上 / 眼在手外 统一）

#### 1.主函数`func()`

**compute.py**（眼在手上 | 眼在手外 统一，按 config.yaml 的 calib_mode 切换）
...
**compute.py**（依据 config.yaml 的 `calib_mode` 自动选择）

```python
poses_to_csv(file_path, "RobotToolPose.csv", invert=INVERT)  # INVERT = (calib_mode == "eye_to_hand")
```

- 眼在手上（eye_in_hand，invert=False）：机械臂末端位姿 → **末端相对基座**的齐次变换矩阵
- 眼在手外（eye_to_hand，invert=True）：对矩阵**求逆** → **基座相对末端**的齐次变换矩阵
...
- 使用OpenCV的 `calibrateHandEye` 函数进行手眼标定
  ...
  - 眼在手上（eye_in_hand）模式：计算**相机**相对于**机械臂末端**的旋转矩阵 **R_cam2end**和平移向量 **T_cam2end**。
  ...
  - 眼在手外（eye_to_hand）模式：计算**相机**相对于**机械臂基座**的旋转矩阵 **R_cam2base** 和平移向量**T_cam2base**。
...
​		运行脚本`compute.py`（config.yaml 设 `calib_mode: eye_in_hand`），获取标定结果
...
运行脚本`compute.py`（config.yaml 设 `calib_mode: eye_to_hand`），获取标定结果
